...
public class MemberController {
    ...
    public ResponseEntity<MemberInfResponse> inquiryMember(@RequestHeader("Authorization") String token, @PathVariable(value="id") Long id){
        jwtService.validateToken(token);
        return ResponseEntity.ok().body(memberService.memberMyPage(id));
    }
    @ApiOperation(value="맴버 질문 조회",notes = "<strong>내가 등록한 질문을 조회한다.</strong>")
    @RequestMapping(value="/api/v1/users/questions/{id}", method = RequestMethod.GET)
    public ResponseEntity<QuestionsResponse> inquiryAnswer(@PathVariable(value="id") Long id){
        return ResponseEntity.ok().body(memberService.memberQuestions(id));
    }
    @ApiOperation(value="맴버 답변 조회",notes = "<strong>내가 등록한 답변을 조회한다.</strong>")
    @RequestMapping(value="/api/v1/users/answers/{id}", method = RequestMethod.GET)
    public ResponseEntity<AnswersResponse> inquiryQuestion(@PathVariable(value="id") Long id){
        return ResponseEntity.ok().body(memberService.memberAnswers(id));
    }
    /*
